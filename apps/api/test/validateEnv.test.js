...
const productionConfig = () => ({
  ...baseConfig(),
  isProduction: true,
  databaseUrl: 'postgresql://user:pass@localhost:5432/telex?sslmode=require',
  ...
  observability: {
    ...
    errorMonitorWebhookUrl: 'https://alerts.example.com/telex',
  },
});
...
test('production error monitor endpoint must use HTTPS', () => {
  const config = productionConfig();
  config.observability.errorMonitorWebhookUrl = 'http://alerts.example.com/telex';
  assert.throws(() => validateEnv(config), /must use HTTPS/);
});
