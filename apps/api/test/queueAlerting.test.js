...
test('evaluateQueueHealth returns healthy when under thresholds', () => {
  ...
  assert.ok(metricsText.includes('telex_queue_backlog_size{queue="whatsapp-messages"} 7'));
  assert.ok(metricsText.includes('telex_queue_waiting_jobs{queue="whatsapp-messages"} 5'));
  assert.ok(metricsText.includes('telex_queue_lag_seconds{queue="whatsapp-messages"} 10'));
});
