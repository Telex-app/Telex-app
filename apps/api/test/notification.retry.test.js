...
test('registerInboundJobs registers the outbound retry processor', async () => {
  const calls = [];
  ...
  delete require.cache[path.resolve(__dirname, '../src/jobs/inbound.jobs.js')];
  const { registerInboundJobs } = require('../src/jobs/inbound.jobs');
  registerInboundJobs();

  assert.deepEqual(calls.map((call) => call.name).sort(), ['messaging-inbound', 'whatsapp-outbound-retry']);
});
...
test('sendTextMessage queues a retry for transient Meta errors and records the notification', async () => {
  ...
  delete require.cache[path.resolve(__dirname, '../src/services/messaging.service.js')];
