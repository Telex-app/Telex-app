...
const anonymizeUser = async (db, options = {}) => {
  ...
  // The Telegram identity bridge maps a real phone number to a chat id, so it
  // is personal data in its own right and must not survive anonymization —
  // otherwise the "deleted" user is still re-identifiable from their chat.
  // Deleted rather than anonymized: unlike the rows below it carries no
  // financial history worth retaining, and a stale link would misroute
  // messages if that chat id is reused.
  if (safeDb.telegramLink?.deleteMany) {
    await safeDb.telegramLink.deleteMany({ where: { phoneNumber: user.phoneNumber } });
  }
