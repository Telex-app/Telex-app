...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    if (origin === 'null') {
      increment('telex_cors_rejected_total', { reason: 'null_origin' });
    ...
    increment('telex_cors_rejected_total', { reason: 'unapproved_origin' });
...
app.use((err, req, res, next) => {
  if (err.type === 'entity.too.large') {
    increment('telex_body_limit_exceeded_total', { route: req.path });
    if (!res.headersSent) {
...
app.get(['/health', '/health/ready'], async (req, res) => {
  ...
  try {
    ...
    increment('telex_health_checks_total', { status: 'ok' });
    res.status(200).json({ status: 'ok', db: 'connected', redis: 'connected', uptime: process.uptime(), correlationId });
  } catch (error) {
    increment('telex_health_checks_total', { status: 'degraded' });
...
// Routes
// Telegram Bot API webhook. Must be mounted before the Meta '/webhook' router:
// app.use matches on prefix, so '/webhook' would otherwise run its body parser
// and error handler over Telegram requests on the way past. Mounted regardless
// of MESSAGE_TRANSPORT so setWebhook registration and health probes always have
// a target; the secret-token middleware is what actually gates it.
