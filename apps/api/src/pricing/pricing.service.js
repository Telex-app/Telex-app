...
const validateProviderPayload = ({ payload, sourceCurrency, targetCurrency, now = new Date() }) => {
  ...
  if (!match || !match[1]) {
    incrementMetric('telex_pricing_provider_failures_total', { provider: 'exchangerate-api', reason: 'invalid_data' });
  ...
  if (now.getTime() - sourceTimestamp.getTime() > Number(config.pricing?.maxSourceAgeMs ?? 86400000)) {
    incrementMetric('telex_pricing_provider_failures_total', { provider: 'exchangerate-api', reason: 'stale_data' });
...
const recordProviderFailure = (provider, reason) => {
  ...
  incrementMetric('telex_pricing_provider_failures_total', { provider, reason });
};

const cachedRate = (key, maxAgeMs) => {
  const cached = rateCache.get(key);
  if (!cached || Date.now() - cached.cachedAt.getTime() > maxAgeMs) return null;
  incrementMetric('telex_pricing_cache_used_total', { provider: cached.provider, freshness: Date.now() - cached.sourceTimestamp.getTime() <= Number(config.pricing?.cacheMaxAgeMs ?? 60000) ? 'fresh' : 'stale' });
  return cached;
};
...
const fetchExchangeRateQuote = async ({ sourceCurrency = 'NGN', targetCurrency = 'USDC' }) => {
  ...
  if (circuit.openedUntil > Date.now()) {
    ...
    incrementMetric('telex_pricing_provider_failures_total', { provider, reason: 'open_circuit' });
...
const validateQuoteForExecution = async ({ quote, userId, asset, amount }) => {
  if (!quote) {
    incrementMetric('telex_quote_rejected_total', { reason: 'not_found' });
  ...
  if (quote.sourceCurrency !== asset || quote.targetCurrency !== asset) {
    incrementMetric('telex_quote_rejected_total', { reason: 'asset_pair' });
  ...
  if (compare(quote.sourceAmount, amount, asset) !== 0) {
    incrementMetric('telex_quote_rejected_total', { reason: 'amount' });
  ...
  if (expectedRate != null && quote.rate != null && String(quote.rate) !== String(expectedRate)) {
    incrementMetric('telex_quote_rejected_total', { reason: 'rate_stale' });
...
const requote = async ({ userId, quoteId, tx, emit = true } = {}) => {
  ...
  if (emit) {
    ...
    incrementMetric('telex_quote_requoted_total', { reason: stale.status === QUOTE_STATUS.EXPIRED ? 'expired' : 'manual' });
  }
...
const reconcileQuotes = async ({ prismaClient, now = new Date(), orphanGraceMs = 60 * 60 * 1000, emit = true } = {}) => {
  ...
  if (emit && (expired.count > 0 || orphaned.count > 0)) {
    ...
    if (expired.count > 0) incrementMetric('telex_quote_expired_total', { reason: 'reconciliation' }, expired.count);
    if (orphaned.count > 0) incrementMetric('telex_quote_expired_total', { reason: 'orphaned' }, orphaned.count);
  }

  return { expired: expired.count, orphaned: orphaned.count };
};

// policyRate is NOT re-exported here. It depends on this module for
// getExchangeRate, so re-exporting its symbols made the two files mutually
// dependent and forced policyRate into a lazy require inside a function body.
// Every consumer (compliance.service, kycDecision.service, the tests) already
// imports getPolicyConversionSnapshot / PolicyError / POLICY_ERROR_CODES from
// pricing/policyRate directly, so the indirection bought nothing.
module.exports = {
  createQuote,
  getExchangeRate,
