...
pool.on('error', (error) => increment('telex_database_pool_errors_total', { message: error.message }));
pool.on('acquire', () => setGauge('telex_database_pool_waiting', pool.waitingCount));
  ...
  if (connection && typeof connection.then === 'function') {
    ...
    const timeout = new Promise((_, reject) => {
      timeoutHandle = setTimeout(() => {
        expired = true;
        increment('telex_database_pool_timeouts_total');
    ...
    return Promise.race([connection, timeout]).finally(() => {
      ...
      observeDuration(
        'telex_database_pool_wait_seconds',
        {},
...
const updatePoolMetrics = () => {
  setGauge('telex_database_pool_total', pool.totalCount);
  setGauge('telex_database_pool_idle', pool.idleCount);
  setGauge('telex_database_pool_waiting', pool.waitingCount);
};
