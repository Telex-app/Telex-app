...
const { recordDeliveryStatus } = require('../services/messaging.service');
...
const startWebhookInboxDrain = ({
  ...
  const runDrain = async () => {
    try {
      ...
      increment('telex_webhook_inbox_backlog', { size: String(stats.backlog) });
...
const startOutboxReconciler = ({
  ...
  const runSweep = async () => {
    try {
      ...
      if (flagged.length) {
        increment('telex_notification_unresolved_total', { count: String(flagged.length) });
      }
    } catch (error) {
