...
const { retryOutboundNotification } = require('../services/messaging.service');
...
const registerInboundJobs = ({ orderingStore } = {}) => {
  if (!orderingStore) {
  ...
  const processInboundMessage = async (job) => {
    const { from, whatsappName, text, mediaId, messageType, whatsappMessageId, channel = 'whatsapp' } = job.data;
    ...
    if (messageType === 'audio' || messageType === 'voice') {
      await processVoiceMessage({ phoneNumber: from, whatsappName, mediaId, whatsappMessageId, channel });
    } else {
  ...
  const inboundWorker = registerProcessor('messaging-inbound', orderedProcessor);
