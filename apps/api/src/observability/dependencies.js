...
const checkDependency = async (dependency, { timeoutMs = config.health.timeoutMs } = {}) => {
  ...
  try {
    ...
    observeDuration('telex_dependency_check_duration_seconds', { dependency: dependency.name }, latencyMs / 1000);
    increment('telex_dependency_checks_total', { dependency: dependency.name, status });
    ...
    const latencyMs = Date.now() - startedAt;
    increment('telex_dependency_checks_total', { dependency: dependency.name, status: STATUS.UNHEALTHY });
    return {
      name: dependency.name,
      criticality: dependency.criticality,
      description: dependency.description,
...
const raiseAlerts = (results) => {
  for (const result of results) {
    ...
    increment('telex_dependency_alerts_total', {
...
const checkAll = async (options = {}) => {
  ...
  for (const dependency of dependencies) {
    setGauge('telex_dependency_up', dependency.status === STATUS.UNHEALTHY ? 0 : 1, {
