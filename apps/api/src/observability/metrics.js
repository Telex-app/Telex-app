...
const renderMetrics = () => {
  const lines = [
    ...
    '# TYPE telex_process_uptime_seconds gauge',
    `telex_process_uptime_seconds ${process.uptime()}`,
    '# HELP telex_process_resident_memory_bytes Resident memory.',
    '# TYPE telex_process_resident_memory_bytes gauge',
    `telex_process_resident_memory_bytes ${process.memoryUsage().rss}`,
  ];
  const names = new Set();
...
const requestMetrics = (req, res, next) => {
  ...
  const capture = () => {
    ...
    increment('telex_http_requests_total', labels);
    observeDuration('telex_http_request_duration_seconds', labels, Number(process.hrtime.bigint() - started) / 1e9);
  };
