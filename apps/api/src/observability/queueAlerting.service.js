...
const evaluateQueueHealth = (queueName, stats = {}) => {
  ...
  setGauge('telex_queue_backlog_size', backlog, { queue: queueName });
  setGauge('telex_queue_waiting_jobs', waiting, { queue: queueName });
  setGauge('telex_queue_active_jobs', active, { queue: queueName });
  setGauge('telex_queue_failed_jobs', failed, { queue: queueName });
  setGauge('telex_queue_delayed_jobs', delayed, { queue: queueName });
  setGauge('telex_queue_lag_seconds', oldestJobAgeSeconds, { queue: queueName });
  ...
  if (backlog >= THRESHOLDS.BACKLOG_CRITICAL) {
    ...
    increment('telex_queue_alerts_total', { queue: queueName, severity: 'critical', type: 'backlog' });
    ...
    increment('telex_queue_alerts_total', { queue: queueName, severity: 'warning', type: 'backlog' });
  }
  ...
  if (oldestJobAgeSeconds >= THRESHOLDS.JOB_LAG_CRITICAL_SECONDS) {
    ...
    increment('telex_queue_alerts_total', { queue: queueName, severity: 'critical', type: 'lag' });
    ...
    increment('telex_queue_alerts_total', { queue: queueName, severity: 'warning', type: 'lag' });
  }
  ...
  if (failed >= THRESHOLDS.FAILED_JOBS_THRESHOLD) {
    ...
    increment('telex_queue_alerts_total', { queue: queueName, severity: 'critical', type: 'failures' });
  }
