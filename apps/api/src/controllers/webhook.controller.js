const { sendTextMessage, recordDeliveryStatus } = require('../services/messaging.service');
...
const processMessage = async ({ message, value }) => {
  ...
  if (!msgValidation.valid) {
    ...
    increment('telex_webhook_events_total', { status: 'invalid_schema' });
    ...
    try {
      ...
      if (err.code === 'P2002') {
        ...
        if (!claimedMessageId) {
          ...
          increment('telex_webhook_events_total', { status: 'duplicate' });
  ...
  if (totalHits > botMax) {
    ...
    increment('telex_webhook_events_total', { status: 'throttled' });
  ...
  try {
    await enqueue('messaging-inbound', 'message.received', {
      from,
      whatsappName,
      ...
      providerTimestamp,
      // Which transport delivered this. Absent for Meta payloads, so the
      // consumer defaults to 'whatsapp' and existing jobs are unaffected; the
      // Telegram normalizer sets it so the voice pipeline knows to fetch media
      // from Telegram's file API instead of the Meta Graph API.
      channel: value?.channel || 'whatsapp',
    }, options);
  } catch (queueError) {
    increment('telex_webhook_events_total', { status: 'failed' });
  ...
  increment('telex_webhook_events_total', { status: 'enqueued' });
  if (claimedMessageId) {
    await prisma.processedMessage.updateMany({
      where: { messageId: claimedMessageId, status: 'claiming' },
...
const handleIncomingMessage = async (req, res) => {
  increment('telex_webhook_events_total', { status: 'received' });
  try {
    const body = req.body;
        ...
        if (Array.isArray(statuses) && statuses.length) {
          ...
          if (wellFormed.length) {
            ...
            if (ingestion.failed.length > 0) {
              increment('telex_webhook_events_total', { status: 'inbox_unavailable' });
              statusIngestionFailed = true;
            } else {
              increment('telex_webhook_events_total', { status: 'status_ingested' });
...
module.exports = {
  ...
  // Exported so the Telegram transport can reuse this pipeline rather than
  // duplicating idempotency, throttling, ordering and claim handling. It
  // consumes a Meta-shaped `value`, which telegram/normalizer.js produces.
  processMessage,
  OUTCOMES,
};
