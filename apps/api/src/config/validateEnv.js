...
const validateEnv = (config) => {
  ...
  if (config.isProduction && config.messageTransport === 'telegram') {
    const requiredTelegram = [
      ['TELEGRAM_BOT_TOKEN', config.telegram?.botToken],
      ['TELEGRAM_WEBHOOK_SECRET', config.telegram?.webhookSecret],
      ['TELEGRAM_CALLBACK_URL', config.telegram?.callbackUrl],
    ];
    for (const [name, value] of requiredTelegram) {
      if (!value) problems.push(`${name} must be set for the production Telegram webhook.`);
    }
    if (config.telegram?.callbackUrl && !config.telegram.callbackUrl.startsWith('https://')) {
      problems.push('TELEGRAM_CALLBACK_URL must use HTTPS in production.');
    }
    // The secret token is the only thing standing between the public webhook
    // URL and forged updates, so hold it to the same length bar as the Meta
    // verify token rather than accepting whatever setWebhook would allow.
    if (config.telegram?.webhookSecret && config.telegram.webhookSecret.length < 32) {
      problems.push('TELEGRAM_WEBHOOK_SECRET must be at least 32 characters in production.');
    }
  }

  if (!['meta', 'telegram', 'sim'].includes(config.messageTransport)) {
    problems.push(`MESSAGE_TRANSPORT must be one of 'meta', 'telegram' or 'sim' (got '${config.messageTransport}').`);
  }
