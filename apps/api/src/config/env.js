...
module.exports = {
  ...
  // Telegram Bot API transport (MESSAGE_TRANSPORT=telegram). Telegram has no
  // notion of a phone number as an address, so `sendTextMessage(to, ...)` still
  // takes an E.164 number and resolves it to a chat id via TelegramLink — see
  // telegram/link.service.js. That keeps every caller in the codebase unchanged.
  telegram: {
    botToken: process.env.TELEGRAM_BOT_TOKEN,
    // Echoed back by Telegram in X-Telegram-Bot-Api-Secret-Token on every
    // webhook POST. This is Telegram's stand-in for Meta's X-Hub-Signature-256:
    // a shared secret, not a body signature, so it is compared in constant time
    // and the transport must be HTTPS.
    ...
    connectTimeoutMs: Number(process.env.TELEGRAM_CONNECT_TIMEOUT_MS || 10000),
    responseTimeoutMs: Number(process.env.TELEGRAM_RESPONSE_TIMEOUT_MS || 10000),
    maxSendRetries: Number(process.env.TELEGRAM_SEND_MAX_RETRIES || 2),
    retryBaseDelayMs: Number(process.env.TELEGRAM_SEND_RETRY_BASE_DELAY_MS || 250),
  },
  // Public origin of the web app that renders receipts, e.g.
  // https://app.example.com. Intentionally unset by default: the verify link is
  // customer-facing, so no link is better than a guessed host. Receipts omit
  // the link entirely when this is empty.
  ...
  observability: {
    serviceName: process.env.SERVICE_NAME || 'telex-api',
